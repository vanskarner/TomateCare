...
fun useMobilenetV2WithAutogeneratedModel(context: Context, imgBitmap: Bitmap): Result<Classification> {
    try {
        ...
        return if (topPrediction.first == CLASSIFICATION_CLASSES[2])
            Result.success(Classification.healthy(topPrediction.second,timeInMillis ,labeledPredictions))
...
fun useMobilenetV2WithInterpreter(context: Context, imgBitmap: Bitmap): Result<Classification> {
    try {
        val outputTensor :TensorBuffer
        val timeInMillis = measureTimeMillis {
...
fun useMobilenetV2WithInterpreter(context: Context, imgList: List<Bitmap>): Result<ClassificationByBatch> {
    try {
        val outputTensor :TensorBuffer
        val timeInMillis = measureTimeMillis {
            val interpreter = Interpreter(loadModelFile(context))
        ...
        outputTensor.floatArray.toList().chunked(10).forEach {
            ...
            if (topPrediction.first == CLASSIFICATION_CLASSES[2])
                predictions.add(ClassificationItem.healthy(topPrediction.second, labeledPredictions))
